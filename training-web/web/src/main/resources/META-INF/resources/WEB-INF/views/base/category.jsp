...
<%--<script>--%>
	...
	var RestDataSource_Sub_Category = isc.RestDataSource.create({
		...
		fetchDataURL: "${restApiUrl}/api/category/dummy"
		<%--fetchDataURL: "${restApiUrl}/api/category/subcategory/{categoryId}/spec-list"--%>
	});
			...
			var Dialog_Delete = isc.Dialog.create({
				...
				buttonClick: function (button, index) {
					...
					if (index == 0) {
						...
						isc.RPCManager.sendRequest({
							actionURL: "${restApiUrl}/api/sub-category/" + record.id,
							httpMethod: "DELETE",
							useSimpleHttp: true,
							contentType: "application/json; charset=utf-8",
							httpHeaders: {"Authorization": "Bearer " + "${cookie['access_token'].getValue()}"},
	...
	function ListGrid_Sub_Category_Add() {
		...
		if (crecord == null || crecord.id == null) {
			...
			method = "POST";
			url = "${restApiUrl}/api/sub-category";
	...
	function ListGrid_Sub_Category_Edit() {
		...
		if (record == null || crecord == null) {
			...
			method = "PUT";
			url = "${restApiUrl}/api/sub-category/" + record.id;
			...
			var Dialog_Delete = isc.Dialog.create({
				...
				buttonClick: function (button, index) {
					...
					if (index == 0) {
						...
						isc.RPCManager.sendRequest({
							actionURL: "${restApiUrl}/api/category/" + record.id,
							httpMethod: "DELETE",
							useSimpleHttp: true,
							contentType: "application/json; charset=utf-8",
							httpHeaders: {"Authorization": "Bearer " + "${cookie['access_token'].getValue()}"},
	...
	function ListGrid_Category_Edit() {
		...
		if (record == null || record.id == null) {
			...
			method = "PUT";
			url = "${restApiUrl}/api/category/" + record.id;
	...
	var ListGrid_Category = isc.ListGrid.create({
		...
		selectionChanged: function (record, state) {
			RestDataSource_Sub_Category.fetchDataURL = "${restApiUrl}/api/category/" + record.id + "/sub-categories";
	...
	var ToolStripButton_Sub_Category_Print = isc.ToolStripButton.create({
	...
	var ToolStrip_Actions_Sub_Category = isc.ToolStrip.create({
		width: "100%",
		members: [ToolStripButton_Sub_Category_Add, ToolStripButton_Sub_Category_Edit, ToolStripButton_Sub_Category_Remove, ToolStripButton_Sub_Category_Refresh, ToolStripButton_Sub_Category_Print]
	});
	...
	var ToolStripButton_Category_Print = isc.ToolStripButton.create({
		icon: "[SKIN]/RichTextEditor/print.png",
		title: "چاپ",
		click: function () {
			window.open("<spring:url value="/category/print/pdf"/>");
	...
	var ToolStrip_Actions_Category = isc.ToolStrip.create({
		width: "100%",
		members: [ToolStripButton_Category_Add, ToolStripButton_Category_Edit, ToolStripButton_Category_Remove, ToolStripButton_Category_Refresh, ToolStripButton_Category_Print]
	});
