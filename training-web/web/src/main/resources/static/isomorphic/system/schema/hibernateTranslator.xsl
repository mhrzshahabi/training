			...
			<xsl:element name="class">
				...
				<xsl:if test="/DataSource/@schema">
					<xsl:attribute name="schema">
						<xsl:value-of select="/DataSource/@schema"/>
					</xsl:attribute>
				</xsl:if>
						...
						<xsl:element name="{$tagName}">
							...
							<xsl:if test="/DataSource/@quotedColumnNames">
								<xsl:if test="/DataSource/@quotedColumnNames = 'true'">
									<xsl:attribute name="column">`<xsl:value-of select="@name"/>`
									</xsl:attribute>
								</xsl:if>
							</xsl:if>
							...
							<xsl:variable name="type">
								<xsl:choose>
									...
									<!-- imageType -->
									<xsl:when test="@type = 'imageFile'">com.isomorphic.hibernate.BlobUserType
									</xsl:when>
									...
									<xsl:when test="@type = 'sequence'">
										<generator class="native">
											<xsl:choose>
												...
												<xsl:otherwise>
													<param name="sequence"><xsl:value-of
															select="/DataSource/@tableName"/>_<xsl:value-of
															select="@name"/>
													</param>
												</xsl:otherwise>
											</xsl:choose>
