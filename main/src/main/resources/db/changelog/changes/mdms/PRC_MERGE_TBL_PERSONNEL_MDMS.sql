...
MERGE INTO TBL_PERSONNEL T
    USING (
        ...
        FROM (SELECT emp.*,
              ...
              FROM (select po.c_username,
                    ...
                    FROM (select *
                       ...
                       LEFT JOIN   tbl_md_employee_STATUS_MDMS stat ON emp.c_emp_status_id = stat.c_id
                       LEFT JOIN   TBL_MD_EMPLOYMENT_TYPE_MDMS type ON emp.c_emp_type_id = type.c_id
                       LEFT JOIN   TBL_MD_POST_MDMS mdms_post on mdms_post.c_id = emp.c_post_id
        ...
        where TR_PERS.personnel_no <> to_char(emp.c_em_number_10)
           ...
           OR TR_PERS.post_grade_code <> emp.jobgrade
           OR (TR_PERS.post_grade_code IS NULL
            AND emp.jobgrade IS NOT NULL)
           OR (TR_PERS.post_grade_code IS NOT NULL
            AND emp.jobgrade IS NULL)
