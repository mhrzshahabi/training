...
//    @PreAuthorize("hasAuthority('c_job_group')")
    public ResponseEntity<GroupOfPersonnelDTO.Info> create(@Validated @RequestBody GroupOfPersonnelDTO.Create request) {
        return new ResponseEntity<>(groupOfPersonnelService.create(request), HttpStatus.CREATED);
    }
