...
//    @PreAuthorize("hasAuthority('c_tclass')")
 ...
 @Loggable
    @GetMapping(value = "/attendance-person/iscList")
    public ResponseEntity<ISC<GenericReport>> attendancePersonReport(HttpServletRequest iscRq) throws IOException {
        SearchDTO.SearchRq searchRq = ISC.convertToSearchRq(iscRq);
        List<SearchDTO.CriteriaRq> criteriaRqList = searchRq.getCriteria().getCriteria();
        ...
        List<GenericReport> reportDTOList = iClassStudentService.getAttendancePersonReport( fromDate, toDate, complex, complexNull, assistant, assistantNull, affairs, affairsNull);
