...
//<script>
    ...
    function showNewForm_needAssessment() {
        ...
        NeedAssessmentWin_needAssessment.setTitle("<spring:message code="create"/>&nbsp;" + "<spring:message
        code="need.assessment"/>");
    ...
    function showEditForm_needAssessment() {
        ...
        if (checkRecordAsSelected(record, true)) {
            ...
            NeedAssessmentWin_needAssessment.setTitle("<spring:message code="edit"/>&nbsp;" + "<spring:message
        code="need.assessment"/>" + '&nbsp;\'' + record.titleFa + '\'');
            NeedAssessmentWin_needAssessment.show();
        }
    };
    ...
    function saveNeedAssessment_needAssessment() {
        ...
        isc.RPCManager.sendRequest(
            TrDSRequest(needAssessmentSaveUrl, needAssessmentMethod_needAssessment, JSON.stringify(data), "callback: studyRcpResponse(rpcResponse, '<spring:message
        code="need.assessment"/>', '" + needAssessmentAction + "')")
        );
    };
        ...
        if (checkRecordAsSelected(record, true)) {
            isc.TrYesNoDialog.create({
                ...
                buttonClick: function (button, index) {
                    ...
                    if (index == 0) {
                        isc.RPCManager.sendRequest(
                            TrDSRequest(needAssessmentUrl + "/" + record.id, "DELETE", null, "callback: studyRcpResponse(rpcResponse, '<spring:message
        code="need.assessment"/>', '<spring:message code="removed"/>')")
                        );
                    }
                }
            });
