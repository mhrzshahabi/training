...
SelectedCompetenceTS_needsAssessment = isc.ToolStrip.create({
members: [
isc.ToolStripButtonRefresh.create({title: "", click: function () { refreshLG(SelectedCompetenceLG_needsAssessment); }}),
]
});
...
<%--    canRemoveRecords: true,--%>
<%--    gridComponents: [--%>
<%--        isc.LgLabel.create({contents: "<span><b>" + "<spring:message code="competences.selected"/>" + "</b></span>", customEdges: ["B"]}),--%>
<%--        SelectedCompetenceTS_needsAssessment, "filterEditor", "header", "body"--%>
<%--    ],--%>
<%--    canAcceptDroppedRecords: true,--%>
<%--    recordDrop(dropRecords, targetRecord, index, sourceWidget) {--%>
<%--        if (sourceWidget.ID === 'CompetenceLG_needsAssessment') {--%>
<%--            this.transferSelectedData(sourceWidget);--%>
<%--        }--%>
<%--    },--%>
<%--});--%>
...
<%--    height: "1%",--%>
<%--    members: [--%>
<%--        isc.LgLabel.create({width: "25%", customEdges: ["B"]}),--%>
...
width: "10%",
showResizeBar: true,
members: [SelectedCompetenceLG_needsAssessment],
}),
isc.TrVLayout.create({
width: "75%",
showResizeBar: true,
members: [
isc.LgLabel.create({height: "1%", contents: "<span><b>" + "<spring:message code="domain"/>" + "</b></span>", customEdges: ["T", "L", "R"]}),
isc.TrHLayout.create({
height: "1%",
members: [
isc.LgLabel.create({width: "33.33%", contents: "<span><b>" + "<spring:message code="knowledge"/>" + "</b></span>"}),
isc.LgLabel.create({width: "33.33%", contents: "<span><b>" + "<spring:message code="ability"/>" + "</b></span>", customEdges: ["T", "B"]}),
isc.LgLabel.create({width: "33.33%", contents: "<span><b>" + "<spring:message code="attitude"/>" + "</b></span>"}),
]
}),
isc.TrHLayout.create({
members: [
...
{
name: "needsAssessmentDomain.title", title: "<spring:message code="domain"/>",
// optionDataSource: "NeedsAssessmentPriorityDS_needsAssessment",
// valueField: "id",
// displayField: "title",
// editorType: "SelectItem",
...
// });
<%--if (checkRecordAsSelected(record, true, "<spring:message code="competences.selected"/>")) {--%>
...
<%--}--%>
    ...
    // NeedsAssessmentDomainDS_needsAssessment.fetchData(null, "console.log(data)");
    // NeedsAssessmentDomainDS_needsAssessment.fetchData(null, "NeedsAssesmentLG_needsAssessment.getFieldByName('needsAssessmentDomain.title').emptyCellValue = 'sdfsdfsdfsdf'");
    ...
    function config() {
        ...
        // isc.DataSource.get("NeedsAssessmentDomainDS_needsAssessment").fetchData(null, " NeedsAssesmentLG_needsAssessment.getFieldByName('needsAssessmentDomain.title').emptyCellValue = NeedsAssessmentDomainDS_needsAssessment.cacheResultSet.allRows[0].title";
        console.log('__________________________________');
        // NeedsAssesmentLG_needsAssessment.getFieldByName("needsAssessmentDomain.title").emptyCellValue = "sdfsdfsdfsdf";
        // NeedsAssesmentLG_needsAssessment.getFieldByName("needsAssessmentDomain.title").emptyCellValue =
        // NeedsAssessmentDomainDS_needsAssessment.fetchData();
        // var t = NeedsAssessmentDomainDS_needsAssessment;
        // console.log(t);
        // console.log(t.cacheResultSet);
        // temp1 = NeedsAssessmentDomainDS_needsAssessment.cacheResultSet.allRows[0].title;
        //  console.log(temp1.cacheResultSet.allRows[0].title);
        // NeedsAssesmentLG_needsAssessment.getFieldByName("needsAssessmentDomain.title").emptyCellValue =
    }
