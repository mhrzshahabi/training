...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
	...
	<xsl:template name="webService" match="/wsdl:definitions">
		...
		<WebService dataURL="{.//wsdl:service//soap:address/@location}" name="{.//wsdl:service[//soap:address]/@name}"
			...
			<xsl:when test="@ref or @element">
				...
				<xsl:choose>
					...
					<xsl:when test="$refElement/xs:complexType">
						<field name="{$refElement/@name}" type="{$refElement/@name}" xmlRequired="{$xmlRequired}"
							   xsElementRef="true">
							<xsl:if test="$debug">
					...
					<xsl:otherwise>
						...
						<field name="{$refLocal}" type="{$refLocal}" mustQualify="true" xmlRequired="{$xmlRequired}"
							   xsElementRef="true">
