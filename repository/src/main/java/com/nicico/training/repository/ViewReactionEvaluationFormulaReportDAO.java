...
public interface ViewReactionEvaluationFormulaReportDAO extends BaseDAO<ViewReactionEvaluationFormulaReport, Long> {
    ...
    @Query(value = "SELECT\n" +
            ...
            "\n" +
            "    Case When darsadJavab.javabdade IS NULL THEN 0 ELSE darsadJavab.javabdade END as javab_dade ,\n" +
            "      CAST( (Case When darsadJavab.javabdade IS NULL THEN 0 ELSE darsadJavab.javabdade END/total_std)*100  AS DECIMAL(8, 2)) AS miangin_javab_dade\n" +
            "FROM\n" +
            "         (\n" +
            "        SELECT\n" +
            "            class_id,\n" +
