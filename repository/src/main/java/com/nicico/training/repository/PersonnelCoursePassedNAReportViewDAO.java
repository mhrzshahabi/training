...
public interface PersonnelCoursePassedNAReportViewDAO extends JpaRepository<PersonnelCoursePassedNAReportView, PersonnelCourseKey>, JpaSpecificationExecutor<PersonnelCoursePassedNAReportView> {
    ...
    @Query(value = "SELECT " +
            ...
            "    NVL(SUM(CASE WHEN na.NA_PRIORITY_ID = 574 THEN 1 ELSE 0 END),0) AS total_essential_appointment_personnel_count, " +
            "    NVL(SUM(CASE WHEN na.NA_PRIORITY_ID = 574 AND na.IS_PASSED = 399 THEN 1 ELSE 0 END),0) AS not_passed_essential_appointment_personnel_count, " +
    ...
    @Query(value = "SELECT " +
            "    NVL(SUM(CASE WHEN na.na_priority_id = 111 THEN 1 ELSE 0 END),0) AS count_essential_service_total, " +
            "    NVL(SUM(CASE WHEN na.na_priority_id = 111 AND na.IS_PASSED = 216 THEN 1 ELSE 0 END),0) AS count_essential_service_passed, " +
            "    NVL(SUM(CASE WHEN na.na_priority_id = 574 THEN 1 ELSE 0 END),0) AS count_essential_appointment_total, " +
            "    NVL(SUM(CASE WHEN na.na_priority_id = 574 AND na.IS_PASSED = 216 THEN 1 ELSE 0 END),0) AS count_essential_appointment_passed, " +
