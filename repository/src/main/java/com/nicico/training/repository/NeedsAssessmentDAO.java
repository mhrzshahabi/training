...
public interface NeedsAssessmentDAO extends BaseDAO<NeedsAssessment, Long> {
    ...
    @Query(value = "update TBL_NEEDS_ASSESSMENT SET N_MAIN_WORKFLOW_STATUS_CODE = :workflowStatusCode , C_MAIN_WORKFLOW_STATUS = :workflowStatus WHERE f_object = (SELECT  f_object FROM TBL_NEEDS_ASSESSMENT WHERE id = :needsAssessmentId)", nativeQuery = true)
    public Integer updateNeedsAssessmentWorkflowMainStatus(Long needsAssessmentId, Integer workflowStatusCode, String workflowStatus);
