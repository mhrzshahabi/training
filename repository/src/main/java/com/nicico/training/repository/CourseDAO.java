...
public interface CourseDAO extends JpaRepository<Course, Long>, JpaSpecificationExecutor<Course> {
    ...
    @Query(value = "select count(*) from training.TBL_COURSE c  where Not EXISTS(select F_COURSE_ID from training.TBL_SKILL_COURSE sc where  sc.F_COURSE_ID=c.ID and sc.F_SKILL_ID = ?)", nativeQuery = true)
