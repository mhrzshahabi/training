...
public interface EquipmentDAO extends JpaRepository<Equipment, Long>, JpaSpecificationExecutor<Equipment> {
    ...
    @Query(value = "select te.* from training.TBL_EQUIPMENT te  where Not EXISTS(select F_EQUIPMENT_ID from training.TBL_TRAINING_PLACE_EQUIPMENT tpe where  tpe.F_EQUIPMENT_ID=te.ID and tie.F_TRAINING_PLACE_ID = ?)", nativeQuery = true)
