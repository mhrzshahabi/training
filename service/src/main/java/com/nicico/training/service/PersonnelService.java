    ...
    public PersonnelDTO.DetailInfo findPersonnel(Long personnelType, Long personnelId, String nationalCode, String personnelNo) {
        ...
        if (personnel != null) {
            ...
            if (synonymPersonnel.getDepartmentId() != null) {
                Optional<Department> optionalDepartment = departmentDAO.findById(synonymPersonnel.getDepartmentId());
                if (optionalDepartment.isPresent())
                    synonymPersonnel.setDepartment(optionalDepartment.get());
            }else if (synonymPersonnel.getDepartmentCode() != null){
...
//            }
    ...
    @Override
    public boolean isPresentByNationalCode(String nationalCode) {
        Optional<Personnel>  personnel = personnelDAO.findFirstByNationalCodeAndActiveAndDeleted(nationalCode,1,0);
        if (personnel.isEmpty()){
            return false;
        }else {
            Optional<PersonnelRegistered>  personnelRegistered = personnelRegisteredDAO.findFirstByNationalCodeAndActive(nationalCode,1);
            return personnelRegistered.isPresent();
        }
